# This file is Forecasting with Target Transformation
...
try:
    ...
    train_df = train_df.set_index(['unique_id','ds']).join(auto_stat_target).reset_index()
...
try:
    baseline_metrics_df = pd.read_pickle(preprocessed/"B0005/ml_single_step_metrics_val_df.pkl")
    baseline_aggregate_metrics_df = pd.read_pickle(preprocessed/"B0005/ml_single_step_aggregate_metrics_val.pkl")
except FileNotFoundError:
    display(HTML("""
    <div class="alert alert-block alert-warning">
...
with open(output_img/"agg_metrics_tt.html", "w") as f:
    f.write(html)
